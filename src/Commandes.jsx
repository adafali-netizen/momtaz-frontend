...
export default function Commandes({ role, nom, navigate }) {
  ...
  async function selectCommande(c) {
    ...
    setEvents([]);
    const { data } = await supabase.from("commande_events").select("*").eq("commande_id", c.id).order("created_at", { ascending: false });
  ...
  async function handleEnregistrer() {
    ...
    try {
      ...
      if (newStatut === "Expédiée" && selected.statut !== "Expédiée") {
        const { data: produit } = await supabase.from("produits").select("id, stock_disponible").eq("nom", selected.produit).single();
        if (produit) {
          await supabase.from("stock_movements").insert([{
            produit_id: produit.id, type: "sortie",
            quantite: selected.quantite || 1,
            source: "expedition", commande_id: selected.id,
          }]);
      ...
      if (newStatut === "Retour reçu" && selected.statut !== "Retour reçu") {
        ...
        if (produit) {
          await supabase.from("stock_movements").insert([{
            produit_id: produit.id, type: "retour",
            quantite: selected.quantite || 1,
            source: "retour_client", commande_id: selected.id,
          }]);
          await supabase.from("produits").update({
      ...
      // ── Log historique ──
      const evtPayload = {
        commande_id: selected.id, ancien_statut: selected.statut,
        nouveau_statut: newStatut, user_nom: "Admin",
      };
      ...
      const { data: newEvents } = await supabase.from("commande_events").select("*").eq("commande_id", selected.id).order("created_at", { ascending: false });
  ...
  return (
    <>
      ...
      {/* ── BARRE RECHERCHE ── */}
      <div style={{ background: "#fff", borderBottom: "2px solid #E2E8F0", padding: "12px 24px", flexShrink: 0, display: "flex", gap: 12, alignItems: "center" }}>
        <div style={{ position: "relative", flex: 1, maxWidth: 360 }}>
          <span style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", fontSize: 14, color: "#94A3B8" }}>🔍</span>
          <input style={{ width: "100%", padding: "9px 14px 9px 36px", background: "#F8FAFC", border: "1.5px solid #E2E8F0", borderRadius: 10, fontSize: 13, color: "#0F172A", outline: "none", boxSizing: "border-box", fontFamily: "inherit" }}
            placeholder="Rechercher par client, téléphone, ville, produit, tracking, ID..."
            value={search} onChange={e => setSearch(e.target.value)} />
          {search && <button onClick={() => setSearch("")} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "#94A3B8", fontSize: 16, padding: 0 }}>×</button>}
        </div>
        <span style={{ fontSize: 12, color: "#94A3B8", marginLeft: "auto" }}>{filtered.length} commande{filtered.length > 1 ? "s" : ""}</span>
        <button onClick={() => setShowModal(true)} style={{ padding: "8px 16px", background: "#2563EB", color: "#fff", border: "none", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>+ Commande</button>
      </div>
        ...
        <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
          <div className="table-wrap">
            <table className="data-table">
              <thead>
                <tr>
                  <th>ID</th><th>Client</th><th>Téléphone</th><th>Produit</th><th>Ville</th>
              ...
              <tbody>
                {filtered.map(c => {
                  ...
                  return (
                    <tr key={c.id} className={selected?.id === c.id ? "selected" : ""} onClick={() => selectCommande(c)}>
                      <td style={{ fontFamily: "monospace", fontSize: 11, color: "#94A3B8" }} title={c.id}>{c.id ? c.id.slice(0, 8) + "…" : "—"}</td>
                      <td style={{ fontWeight: 600 }}>{c.client_nom || "—"}</td>
                      <td className="col-mono">{c.telephone}</td>
                      <td>{c.produit || "—"}</td>
                      <td className="col-muted">{c.ville || "—"}</td>
                      <td className="col-muted">{c.transporteur || <span style={{ color: manqueExpedition ? "#DC2626" : "#CBD5E1" }}>{manqueExpedition ? "⚠ manquant" : "—"}</span>}</td>
                      <td className="col-mono col-muted">{c.tracking || <span style={{ color: manqueExpedition ? "#DC2626" : "#CBD5E1" }}>{manqueExpedition ? "⚠ manquant" : "—"}</span>}</td>
            ...
            <aside style={{ width: 360, flexShrink: 0, background: "#fff", borderLeft: "1px solid #E2E8F0", display: "flex", flexDirection: "column", overflow: "hidden" }}>
              <div style={{ padding: "18px 20px 14px", borderBottom: "1px solid #E2E8F0", background: "#FAFAFA", flexShrink: 0 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                  ...
                  <button onClick={() => setSelected(null)} style={{ background: "#fff", border: "1px solid #E2E8F0", borderRadius: 6, color: "#64748B", fontSize: 16, cursor: "pointer", padding: "2px 8px" }}>×</button>
                </div>
              ...
              <div style={{ flex: 1, overflowY: "auto" }}>
                <div style={{ padding: "16px 20px", display: "flex", flexDirection: "column", gap: 14 }}>
                  ...
                  <div>
                    ...
                    <select value={newStatut} onChange={e => { setNewStatut(e.target.value); setErrors({}); }} disabled={saving}
                      style={{ width: "100%", padding: "10px 12px", background: newStatutMeta.bg, border: `1.5px solid ${newStatutMeta.color}55`, borderRadius: 8, fontSize: 13, fontWeight: 600, color: newStatutMeta.color, cursor: "pointer", outline: "none", fontFamily: "inherit" }}>
                      {STATUTS_CMD.map(s => <option key={s} value={s}>{S_CMD[s].emoji} {s}</option>)}
                    </select>
                  </div>

                  <div>
                    <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".08em", color: "#94A3B8", marginBottom: 6 }}>Date — {newStatut}</div>
                    <input type="date" value={dateStatut} onChange={e => setDateStatut(e.target.value)} disabled={saving} style={inputStyle(false)} />
                  </div>
                  ...
                  <div>
                    ...
                    <select value={transporteur} onChange={e => { setTransporteur(e.target.value); setErrors(p => ({ ...p, transporteur: false })); }} disabled={saving}
                      style={{ ...inputStyle(errors.transporteur), padding: "9px 10px" }}>
                  ...
                  <div>
                    ...
                    <input type="text" placeholder="Ex: SD123456789MA" value={trackingVal}
                      onChange={e => { setTrackingVal(e.target.value); setErrors(p => ({ ...p, tracking: false })); }}
                      disabled={saving} style={{ ...inputStyle(errors.tracking), fontFamily: "monospace" }} />
                  ...
                  <div>
                    ...
                    <input type="number" placeholder="25" value={fraisLivr}
                      onChange={e => { setFraisLivr(e.target.value); setErrors(p => ({ ...p, fraisLivr: false })); }}
                      disabled={saving} style={inputStyle(errors.fraisLivr)} />
                    {errors.fraisLivr && <div style={{ fontSize: 11, color: "#DC2626", marginTop: 3 }}>⚠ Frais de livraison requis</div>}
                  </div>

                  {newStatut === "Facturée" && (
                    <div>
                      <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".08em", color: errors.fraisEmb ? "#DC2626" : "#94A3B8", marginBottom: 6 }}>
                        Frais emballage / stockage (MAD) <span style={{ color: "#DC2626" }}>*</span>
                      </div>
                      <input type="number" placeholder="5" value={fraisEmb}
                        onChange={e => { setFraisEmb(e.target.value); setErrors(p => ({ ...p, fraisEmb: false })); }}
                        disabled={saving} style={inputStyle(errors.fraisEmb)} />
                      {errors.fraisEmb && <div style={{ fontSize: 11, color: "#DC2626", marginTop: 3 }}>⚠ Frais emballage/stockage requis</div>}
                      <div style={{ fontSize: 11, color: "#94A3B8", marginTop: 3 }}>Frais déduits par Sendit sur cette commande</div>
                    </div>
                  )}
                  ...
                  {showFraisRetour && (
                    <div>
                      ...
                      <input type="number" placeholder="15" value={fraisRet} onChange={e => setFraisRet(e.target.value)} disabled={saving} style={inputStyle(false)} />
                    </div>
                  )}
                  ...
                  <button onClick={handleEnregistrer} disabled={saving}
                    style={{ width: "100%", padding: "12px", background: saving ? "#F8FAFC" : "#534AB7", color: saving ? "#94A3B8" : "#fff", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: saving ? "not-allowed" : "pointer" }}>
                    {saving ? "Enregistrement…" : "Enregistrer"}
                  </button>

                  <div style={{ padding: "0 0 20px" }}>
                    <div style={{ height: "0.5px", background: "#E2E8F0", margin: "4px 0 16px" }} />
                    <div style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: ".08em", color: "#94A3B8", marginBottom: 12 }}>Historique</div>
                    {events.length === 0 ? (
                      <div style={{ fontSize: 12, color: "#94A3B8", fontStyle: "italic", textAlign: "center", padding: "12px 0" }}>Aucun historique</div>
                    ) : (
                          ...
                          return (
                            <div key={ev.id || i} style={{ display: "flex", gap: 10, paddingBottom: 14 }}>
                              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: 16, flexShrink: 0 }}>
                                ...
                                {i < events.length - 1 && <div style={{ width: 1, flex: 1, background: "#E2E8F0", marginTop: 3 }} />}
                              </div>
                              <div style={{ flex: 1 }}>
                                <div style={{ display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap", marginBottom: 3 }}>
                                  <span style={{ fontSize: 11, fontWeight: 700, color: evMeta.color }}>{evMeta.emoji} {ev.nouveau_statut}</span>
                                ...
                                {ev.tracking && <div style={{ fontSize: 11, color: "#64748B", fontFamily: "monospace" }}>🔍 {ev.tracking}</div>}
                                {ev.transporteur && <div style={{ fontSize: 11, color: "#64748B" }}>🚚 {ev.transporteur}</div>}
                              </div>
                            </div>
                          );
                        })}
