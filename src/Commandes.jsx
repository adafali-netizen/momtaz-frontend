...
export default function Commandes({ role, nom, navigate }) {
  ...
  async function handleEnregistrer() {
    ...
    try {
      ...
      // Relevé bancaire frais emballage/stockage (au passage Facturée)
                  ...
                  return (
                    <tr key={c.id} className={selected?.id === c.id ? "selected" : ""} onClick={() => selectCommande(c)}>
                      ...
                      <td className="col-muted">
                        {c.transporteur ? c.transporteur : <span style={{ color: manqueExpedition ? "#DC2626" : "#CBD5E1" }}>{manqueExpedition ? "⚠ manquant" : "—"}</span>}
                      </td>
                      <td className="col-mono col-muted">
                        {c.tracking ? c.tracking : <span style={{ color: manqueExpedition ? "#DC2626" : "#CBD5E1" }}>{manqueExpedition ? "⚠ manquant" : "—"}</span>}
                      </td>
                ...
                <div style={{ padding: "16px 20px", display: "flex", flexDirection: "column", gap: 14 }}>
                  ...
                  <div>
                    ...
                    <select value={transporteur} onChange={e => { setTransporteur(e.target.value); setErrors(p => ({ ...p, transporteur: false })); }} disabled={saving}
                      style={{ ...inputStyle(errors.transporteur), padding: "9px 10px", background: errors.transporteur ? "#FEF2F2" : "#F8FAFC" }}>
                  ...
                  {/* Frais emballage/stockage — uniquement au statut Facturée */}
                  {newStatut === "Facturée" && (
                    <div>
                      ...
                      {errors.fraisEmb && <div style={{ fontSize: 11, color: "#DC2626", marginTop: 3 }}>⚠ Frais emballage/stockage requis pour ce statut</div>}
                  ...
                  {showFraisRetour && (
                    <div>
                      ...
                      <input type="number" placeholder="15" value={fraisRet}
                        onChange={e => setFraisRet(e.target.value)} disabled={saving} style={inputStyle(false)} />
                    </div>
                  )}
                  ...
                  <button onClick={handleEnregistrer} disabled={saving}
                    style={{ width: "100%", padding: "12px", background: saving ? "#F8FAFC" : "#534AB7", color: saving ? "#94A3B8" : "#fff", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: saving ? "not-allowed" : "pointer", transition: "background .15s" }}>
                  ...
                  <div style={{ padding: "0 20px 20px" }}>
                    ...
                    {events.length === 0 ? (
                      <div style={{ fontSize: 12, color: "#94A3B8", fontStyle: "italic", textAlign: "center", padding: "12px 0" }}>Aucun historique pour cette commande</div>
                    ) : (
                          ...
                          return (
                            <div key={ev.id || i} style={{ display: "flex", gap: 10, paddingBottom: 14 }}>
                              ...
                              <div style={{ flex: 1, paddingBottom: 2 }}>
                                ...
                                {ev.note && <div style={{ fontSize: 11, color: "#475569", fontStyle: "italic", marginTop: 2 }}>"{ev.note}"</div>}
                              </div>
                            </div>
                          );
                        })}
