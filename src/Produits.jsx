      ...
      {loading ? (
        ...
        <div className="table-wrap">
          <table className="data-table">
            <thead>
              <tr>
                <th>Produit</th><th>Prix achat</th><th>Prix vente</th><th>Marge</th>
                <th>Fournisseur</th><th>Stock</th><th>Décision</th><th></th>
              </tr>
            </thead>
            <tbody>
              {filtered.map(p => {
                ...
                return (
                  <tr key={p.id}>
                    ...
                    <td className="col-mono">{p.prix_vente ? `${p.prix_vente} MAD` : <span style={{ color: "var(--muted2)" }}>—</span>}</td>
                    <td>{p.prix_vente && p.cout_achat ? <span style={{ fontFamily: "JetBrains Mono", fontWeight: 700, color: marge > 0 ? "var(--green)" : "var(--red)" }}>{marge} MAD</span> : <span style={{ color: "var(--muted2)" }}>—</span>}</td>
                    <td className="col-muted">{p.fournisseur || <span style={{ color: "var(--muted2)" }}>—</span>}</td>
                    <td><span className="col-mono" style={{ fontWeight: 700, color: p.stock_disponible <= 0 ? "var(--red)" : p.stock_disponible < p.stock_minimum ? "var(--orange)" : "var(--green)" }}>{p.stock_disponible} u</span></td>
                    <td><span className="decision-badge" onClick={() => toggleDecision(p.id, p.decision)} style={{ color: d.color, background: d.bg, cursor: "pointer" }}>{d.label}</span></td>
                    <td><button className="btn btn-secondary btn-sm" onClick={() => setEditProduit(p)}>✏️</button></td>
                  </tr>
                );
              })}
            </tbody>
