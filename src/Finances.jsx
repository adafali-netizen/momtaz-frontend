...
// ─── Catégories métier ────────────────────────────────────────────────────────
const CATEGORIES = [
  { key: "DEPENSE_ADS",               label: "Ads",                   color: "#1877f2", sens: "debit"  },
  { key: "CREATION_CONTENU",          label: "Créatives",             color: "#7C3AED", sens: "debit"  },
  { key: "FRAIS_LIVRAISON",           label: "Livraison",             color: "#D97706", sens: "debit"  },
  { key: "FRAIS_CONFIRMATION",        label: "Frais de confirmation", color: "#0891B2", sens: "debit"  },
  { key: "ACHAT_STOCK",               label: "Stock",                 color: "#DC2626", sens: "debit"  },
  { key: "DEPENSE_OPS",               label: "Ops",                   color: "#94A3B8", sens: "debit"  },
  { key: "ENCAISSEMENT_TRANSPORTEUR", label: "Encaissement",          color: "#16A34A", sens: "credit" },
  { key: "APPORT_CAPITAL",            label: "Apport capital",        color: "#534AB7", sens: "credit" },
  { key: "AJUSTEMENT",                label: "Ajustement",            color: "#64748b", sens: "both"   },
];
...
function FormulaireNouveauMouvement({ conseilleres, produits, onSaved, onCancel }) {
  ...
  const [form, setForm] = useState({
    ...
    observation: "", montant: "", sens: "debit", produit_id: "",
  });
  ...
  function handleType(type) {
    ...
    setForm(f => ({ ...f, type, categorie: cat, libelle: lib, libelle_custom: "", sens }));
  }
    ...
    <div style={{ background: CLR.card, border: `1px solid ${CLR.indigoBorder}`,
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <button onClick={handleSave} disabled={saving || !form.montant}
          style={{ padding: "8px 18px", background: !form.montant ? CLR.border : CLR.indigo,
                   color: !form.montant ? CLR.textMuted : "#fff", border: "none",
                   borderRadius: 7, fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
          {saving ? "Enregistrement…" : "Enregistrer"}
        </button>
        <button onClick={onCancel}
          style={{ padding: "8px 14px", background: "transparent", border: `1px solid ${CLR.border}`,
                   borderRadius: 7, fontSize: 13, color: CLR.textSecond, cursor: "pointer" }}>
          Annuler
        </button>
      </div>
    </div>
...
export default function Finances() {
  ...
  // ── Calculs ────────────────────────────────────────────────────────────────
  const soldeBancaire = mouvements.reduce((s, m) => s + getMontant(m), 0);
  ...
  const mvtsFiltres = filterType === "tous"
    ? mvtsPeriode
    : mvtsPeriode.filter(m => m.type === filterType);

  const totalCredits = mvtsPeriode.filter(m => getMontant(m) > 0).reduce((s, m) => s + getMontant(m), 0);
  const totalDebits  = mvtsPeriode.filter(m => getMontant(m) < 0).reduce((s, m) => s + Math.abs(getMontant(m)), 0);
  const soldePeriode = totalCredits - totalDebits;

  const recapCats = CATEGORIES.map(cat => {
    const lignes = mvtsPeriode.filter(m => m.type === cat.key);
    const total  = lignes.reduce((s, m) => s + Math.abs(getMontant(m)), 0);
    return { ...cat, total, count: lignes.length };
  }).filter(c => c.total > 0);
  ...
  return (
    <div style={{ fontFamily: "Inter, system-ui, sans-serif", padding: "0 0 48px" }}>
      ...
      <div style={{
        ...
        <div style={{ fontSize: 12, color: CLR.textMuted }}>
          Tous mouvements enregistrés · {mouvements.length} entrées au total
        </div>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
        ...
        <div style={{ marginLeft: "auto", display: "flex", gap: 16, fontSize: 13, alignItems: "center" }}>
          <span style={{ color: CLR.green, fontWeight: 600 }}>+{fmtMAD(totalCredits)}</span>
          <span style={{ color: CLR.red,   fontWeight: 600 }}>−{fmtMAD(totalDebits)}</span>
          <span style={{ color: soldePeriode >= 0 ? CLR.green : CLR.red, fontWeight: 700,
                         fontFamily: "'JetBrains Mono','Courier New',monospace" }}>
            = {fmtMAD(soldePeriode, true)}
          </span>
        </div>
      </div>
            ...
            {recapCats.map(cat => {
              ...
              return (
                <div key={cat.key} onClick={() => setFilterType(active ? "tous" : cat.key)}
                  ...
                  <div style={{ fontSize: 13, fontWeight: 700, minWidth: 120, textAlign: "right",
        ...
        {loading ? (
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr>
                  {["Date", "Catégorie", "Libellé", "Observation", "Montant", ""].map(h => (
